...
class Trail(Entity):
    def __init__(self, ship, offset_x, offset_y):
        ...
        self.engine_trail_dimensions = Vector2(130 * self._ship.size, 344 * self._ship.size)
    ...
    def draw(self, screen):
        ...
        self.engine_trail.shader.set_uniform_1f('mul_r', 0 if self._isBoosting else 0.49)
        self.engine_trail.shader.set_uniform_1f('mul_g', .9 if self._isBoosting else 0.332)
        self.engine_trail.shader.set_uniform_1f('mul_b', .9 if self._isBoosting else 0.059)
    ...
    def update(self, game_speed, trigger_intensity, boost):
        ...
        """
        self.engine_trail.size = self.engine_trail_dimensions * (1 if boost else trigger_intensity)
